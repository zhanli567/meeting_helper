...
public class AwardCeremonyImportStrategy extends AbstractParticipantImportStrategy {
    ...
    public ImportModels.TemplateDescriptor descriptor() {
        return new ImportModels.TemplateDescriptor(
                ...
                1,
                List.of(
                        new ImportModels.SheetDescriptor(PARTICIPANT_SHEET, true, "一人一行"),
                        new ImportModels.SheetDescriptor(AWARD_SHEET, true, "一条获奖记录一行")
                )
        );
    }
    ...
    public ImportModels.ParsedWorkbook parse(XSSFWorkbook workbook) {
        ...
        var awardRows = new ArrayList<ImportModels.AwardRow>();
        var sheet = workbook.getSheet(AWARD_SHEET);
        if (sheet == null) {
            errors.add("缺少必需工作表：获奖记录");
            return new ImportModels.ParsedWorkbook(participants.rows(), awardRows, errors);
        }
        ...
        for (int index = 1; index <= sheet.getLastRowNum(); index++) {
            ...
            awardRows.add(new ImportModels.AwardRow(
                    index + 1,
                    employeeNo,
                    batchOrder,
                    batchName,
