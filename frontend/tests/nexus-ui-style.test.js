...
test('主要列表容器固定高度并在内部滚动', async () => {
  ...
  assert.match(editDialog, /class="record-table-wrap"/)
  assert.match(editDialog, /:height="recordTableHeight"/)
  assert.match(editDialog, /scrollbar-always-on/)
  assert.match(editDialog, /\.record-table-wrap\s*\{[\s\S]*overflow:\s*hidden;/)
