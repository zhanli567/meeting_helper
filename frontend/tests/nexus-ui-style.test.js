...
test('主要列表容器固定高度并在内部滚动', async () => {
  ...
  const recordTable = await readSource('src/components/ParticipantRecordTable.vue')
  ...
  assert.match(addDialog, /class="add-participant-dialog"/)
  assert.match(addDialog, /class="import-preview-section"/)
  assert.match(addDialog, /\.import-preview-section\s*\{[\s\S]*max-height:[\s\S]*overflow-y:\s*auto;/)
  assert.match(editDialog, /class="participant-form-scroll"/)
  assert.match(editDialog, /class="edit-participant-dialog"/)
  assert.match(editDialog, /height:\s*min\(78vh,\s*720px\);/)
  assert.match(editDialog, /\.participant-form-scroll\s*\{[\s\S]*height:\s*100%;[\s\S]*overflow-y:\s*auto;/)
  assert.match(recordTable, /class="record-table-wrap"/)
  assert.match(recordTable, /:height="tableHeight"/)
  assert.match(recordTable, /scrollbar-always-on/)
  assert.match(recordTable, /\.record-table-wrap\s*\{[\s\S]*overflow:\s*hidden;/)
  ...
  assert.match(regionPanel, /\.marker-detail-section\s*\{[\s\S]*overflow-y:\s*auto;/)
...
test('人员侧栏和新增弹窗使用统一的白底细边框工作台视觉', async () => {
  ...
  assert.match(addDialog, /class="import-card import-card--template"/)
  assert.match(addDialog, /class="upload-surface"/)
})
