...
test('编辑人员弹窗工号只读并使用横向表格编辑记录和字段', async () => {
  ...
  assert.match(source, /:height="recordTableHeight"/)
  assert.match(source, /scrollbar-always-on/)
  assert.match(source, /scrollToNewestColumn/)
  assert.doesNotMatch(source, /\.record-table-wrap\s*\{[\s\S]*overflow:\s*auto;/)
