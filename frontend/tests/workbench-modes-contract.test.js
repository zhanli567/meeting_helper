...
test('三种工作台模式初始缩放和画布背景保持一致', async () => {
  ...
  assert.match(shared, /align-items:\s*flex-start;/)
  assert.match(shared, /justify-content:\s*flex-start;/)
...
test('场馆和会议画布复用统一画布视口组件', async () => {
  ...
  assert.doesNotMatch(shared, /scrollWidth - viewport\.clientWidth\) \/ 2/)
