...
test('三种工作台模式初始缩放和画布背景保持一致', async () => {
  ...
  assert.match(shared, /const resetSignature = computed\(\(\) => resetKeySignature\(props\.resetKey\)\)/)
  assert.match(shared, /watch\(resetSignature,\s*resetViewportAfterRender\)/)
  assert.doesNotMatch(shared, /watch\(\(\) => props\.resetKey/)
