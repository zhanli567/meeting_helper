...
- PostgreSQL 保存正式运行数据，建库时手工执行统一建表脚本，Spring Boot 启动时只校验结构；人员扩展属性、布局样式和版本快照使用 JSON 文本保存，以兼顾结构稳定性和场景扩展性。
...
- 数据库 SQL 源文件固定为仓库根目录 `DDL/meeting_helper.sql`，由数据库管理员或开发人员在建库时执行，不作为 Maven 资源复制到应用类路径。
...
- 系统预置场馆通过 `venue/preset` 下的目录、定义和存储类维护，不向 DDL 添加初始化数据；用户自定义场馆仍保存在 PostgreSQL。
